...
// part of code is from https://github.com/s3team/CryptoHunt
    ...
    void SEEngine::getFormulaLength(std::shared_ptr<BitVector> v, uint32_t &len) {
    ...
    std::shared_ptr<BitVector> SEEngine::getRegister(x86::x86_reg reg) {
        ...
        switch (type) {
            ...
            case HALF: {
                ...
                std::shared_ptr<BitVector> temp_mask = std::make_shared<BitVector>(CONCRETE, mask);
                std::shared_ptr<BitVector> v1 = buildop2("and", origin, temp_mask);
                return v1;
            }
            case QHIGH: {
                auto index = Registers::getRegIndex(reg);
                ...
                std::shared_ptr<BitVector> temp_imm = std::make_shared<BitVector>(CONCRETE, 0x8);
                std::shared_ptr<BitVector> v_shift = buildop2("shr", v1, temp_imm);
                return v_shift;
            }
        }
    }

    std::shared_ptr<BitVector> SEEngine::getMemory(t_type::T_ADDRESS memory_address, t_type::T_SIZE size) {
        ...
        if (size == T_BYTE_SIZE * T_WORD) {
            std::shared_ptr<BitVector> temp_mask = std::make_shared<BitVector>(CONCRETE, 0x0000ffff);
            std::shared_ptr<BitVector> v1 = buildop2("and", v0, temp_mask);
            return v1;
        }

        std::shared_ptr<BitVector> temp_mask = std::make_shared<BitVector>(CONCRETE, 0x000000ff);
        std::shared_ptr<BitVector> v1 = buildop2("and", v0, temp_mask);
        return v1;
    }
    ...
    SEEngine::Extract(std::shared_ptr<BitVector> v, int low, int high) {
        assert(high > low);
        std::shared_ptr<BitVector> res = buildop1("bvextract", v);
        res->high_bit = high;
        res->low_bit = low;
        return res;
    }
    ...
    uint32_t
    SEEngine::eval(std::shared_ptr<BitVector> v) { //for no input
    ...
    uint32_t
    SEEngine::eval(std::shared_ptr<BitVector> v, std::map<std::shared_ptr<BitVector>, uint32_t> *inmap) {
        const std::unique_ptr<Operation> &op = v->opr;
        if (op == nullptr) {
            if (v->valty == CONCRETE)
    ...
    bool Dyn_X86_INS_MOV::symbolic_execution(tana::SEEngine &se)
    {
        ...
        if (op0->type == Operand::Mem) {
            if (op1->type == Operand::ImmValue) { // mov dword ptr [ebp+0x1], 0x1111
                uint32_t temp_concrete = stoul(op1->field[0], 0, 16);
                se.writeMemory(this->memory_address, std::make_shared<BitVector>(CONCRETE, temp_concrete, se.isImmSym(temp_concrete)));
                return true;
    ...
    bool Dyn_X86_INS_ADC::symbolic_execution(tana::SEEngine &se)
    {
        ...
        if (op0->type == Operand::Reg) { // dest op is reg
            v0 = se.readRegister(se.getRegName(op0->field[0]));
            res = buildop2("add", v0, v1);
            if(CF)
            {
                res = buildop2("add", res, v_one);
            }
            se.writeRegister(se.getRegName(op0->field[0]), res);
            return true;
        } else if (op0->type == Operand::Mem) { // dest op is mem
