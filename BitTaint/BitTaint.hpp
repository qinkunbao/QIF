  ...
  namespace tana {
      namespace bittaint {
          ...
          private:
              ...
              Memory mem;
              Register reg;
              std::vector<tana::Inst_Base>::iterator eip;
              bool forward = true;
              std::vector<tana::Inst_Base>::iterator start;
              std::vector<tana::Inst_Base>::iterator end;
              ...
              int DO_X86_INS_CALL(const tana::Inst_Base &it);

              int DO_X86_INS_PUSH(const tana::Inst_Base &it);

              int DO_X86_INS_POP(const tana::Inst_Base &it);

              int DO_X86_INS_NEG(const tana::Inst_Base &it);

              int DO_X86_INS_NOT(const tana::Inst_Base &it);

              int DO_X86_INS_INC(const tana::Inst_Base &it);

              int DO_X86_INS_DEC(const tana::Inst_Base &it);

              int DO_X86_INS_MOVSX(const tana::Inst_Base &it);

              int DO_X86_INS_MOVZX(const tana::Inst_Base &it);

              int DO_X86_INS_CMOVB(const tana::Inst_Base &it);

              int DO_X86_INS_MOV(const tana::Inst_Base &it);

              int DO_X86_INS_LEA(const tana::Inst_Base &it);

              int DO_X86_INS_XCHG(const tana::Inst_Base &it);

              int DO_X86_INS_SUB(const tana::Inst_Base &it);

              int DO_X86_INS_SBB(const tana::Inst_Base &it);

              int DO_X86_INS_IMUL(const tana::Inst_Base &it);

              int DO_X86_INS_MUL(const tana::Inst_Base &it);

              int DO_X86_INS_SHR(const tana::Inst_Base &it);

              int DO_X86_INS_SHL(const tana::Inst_Base &it);

              int DO_X86_INS_SHLD(const tana::Inst_Base &it);

              int DO_X86_INS_SHRD(const tana::Inst_Base &it);

              int DO_X86_INS_ADD(const tana::Inst_Base &it);

              int DO_X86_INS_ADC(const tana::Inst_Base &it);

              int DO_X86_INS_RET(const tana::Inst_Base &it);

              int DO_X86_INS_AND(const tana::Inst_Base &it);

              int DO_X86_INS_LEAVE(const tana::Inst_Base &it);

              int DO_X86_INS_XOR(const tana::Inst_Base &it);

          public:
              BitTaint(std::string symbol_name, uint32_t memory_address, uint32_t size, \
          std::vector<tana::Inst_Base>::iterator start, std::vector<tana::Inst_Base>::iterator end);

              BitTaint(std::string symbol_name, uint32_t memory_address, uint32_t size, bool forward);

              int execute_insn(std::vector<tana::Inst_Base>::iterator it);
