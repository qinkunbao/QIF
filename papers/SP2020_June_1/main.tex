...
\begin{abstract}
    Side-channel attacks allow adversaries to infer sensitive information based on
    non-functional characteristics.  Existing work on software  side-channel
    detections can identify many potential vulnerabilities.
    %side-channel leakage sites.
    However, in practice many such vulnerabilities leak negligible amount of
    sensitive information and thus developers are often reluctant to address them.
    %Those leakage sites can leak one bit, two bits, or many bits.
    On the other hand, no existing tools can precisely report the number of leaked
