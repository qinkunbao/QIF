...
Finally we run Monte Carlo (MC) simulation to estimate the amount of leaked
information leakage. 
...
\begin{enumerate}
    ...
    dynamic program analysis. We follow their approach and run the target binary with 
    ...
    attacker's observation from side-channel vulnerabilities with math
    formulas. Each formula capture the fined-grained information between input secrets
    and leakage sites. For the consideration of precision and performance, 
    ...
    input key. We use random testing instead of SMT solvers to find satisfying 
    variables.
    The above design significantly reduces the overhead of symbolic
    executions, which makes the tool scales to real-world programs.
    ...
    which models the observations from attackers. We propose a Monte Carlo method to 
    estimate the number of satisfied solutions. With the help of central limit theorems (CLT),
...
simplify the implementation of symbolic execution. 
