...
a logarithmic function. Second, as the constraint could be very complicated. The
exact counting approach couldn't halt in a reasonable time.

We apply the "counting by sampling" method, which isn't a brand new idea~\cite{10.1007/11499107_24}. 
We extend Wei's method to count the number of solutions for our constrains.

\textit{Random Walk}
(RW) is a local search heuristic methods. We start the search from
