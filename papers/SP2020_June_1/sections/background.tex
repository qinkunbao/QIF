...
component (e.g., the CPU cache, the TLB and DRAM) in modern computer systems.
Depending on the layer causing side-channels, we can classify them 
into the following types of side-channels attacks.

Cached-based side-channels rely on the time differences 
...
FLUSH+RELOAD targets a single cache line. 
It requires the attacker and victim share the same memory address space.
...
Another type of attacks are memory-based side-channels~\cite{7163052} that exploits 
the different behaviors when the
program accesses different memory address. For example, the controlled-channel attack~\cite{7163052},
which works in the shielding system, can infer the sensitive data in the shielding systems by
