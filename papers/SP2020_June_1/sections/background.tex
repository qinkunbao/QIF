\section{Background}
In this section, we first present a basic introduction to the software-based 
side-channel attacks. These attacks are what we study in the paper. 
After that, we will discuss existing methods of side-channel detection 
and quantification.
...
referenced in main memory. If the data does not exist in the cache, 
the CPU will read the data from the main memory (RAM). 
According to the layer that causes the side-channels, 
we classify
side channels into two categories: 
cache-based side-channel attacks and memory-based 
