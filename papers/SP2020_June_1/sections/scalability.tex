...
is huge and the semantics of each instruction is complex. 
Intel Developer Manual~\cite{intelsys}
introduces more than 1000 different X86 instructions. 
It is tedious to manually implement the
...
The IR typically has fewer 
instructions compared to original machine instructions. 
The IR layer designs, which significantly
simplify the implementation, also introduce significant 
overhead as well~\cite{217563}. First, transferring machine
...
transfers into 18 REIL IR instructions. If we assume the time of 
symbolically executing one instruction is constant, the design of 
adopting IR layers can introduce large overhead. 
...
\begin{table}[ht]
      ...
      \caption{The number of X86 instructions and the number
              ...
              It shows that 
              eliminating IR layer can reduce the number
...
\subsubsection{Constriant Slovers}
