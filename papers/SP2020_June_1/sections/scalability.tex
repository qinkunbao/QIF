...
\begin{table}[ht]
      ...
      \begin{tabular}{ccc}
      \hline
                        & \begin{tabular}[c]{@{}c@{}}Number of \\ REIL IR\end{tabular} & \begin{tabular}[c]{@{}c@{}}Number of \\ x86 Instructions\end{tabular} \\ \hline
      AES openSSL 0.9.7 &                                                              &                                                                       \\
      DES openSSL 0.9.7 &                                                              &                                                                       \\
      RSA openSSL 0.9.7 &                                                              &                                                                       \\
      RSA mbed TLS 2.5  &                                                              &                                                                       \\ \hline
      \end{tabular}
      \caption{The number of X86 instructions and the number
      of REIL IR instructions on each trace of crypto programs.}
      \label{scala:ir}
      \end{table}

\subsubsection{Constraint Solvers}
As discussed in section~\ref{side-channel:condition}, the problem
of identifying side-channels can be reduced to the below 
questions.
...
Suppose a program has $n$ byte secrets, the number of total possible input keys is $2^{8m}$. 
We randomly pick up $M$ values and check how many of them satisfy those constrains. 
If $l$ values satisfy those formulas, the approximate result is $\frac{l*2^{8m}}{M}$.

However, the number of satisfying values could be exponentially small. Consider the formula
$f_i={k_1} = 1\land{k_2} = 2\land{k_3} = 3\land{k_4} = 4$, $k_1$, $k_2$, $k_3$ and $k_4$ all represents
one byte in the original sensitive input buffer, 
there is only one satisfying solution of total $2^{32}$ possible
values, which requires exponentially many samples to get a tight bound according to Chernoff bound. 
Monte Carlo method also suffers from the curse of dimensionality. For example, 
the private key length of RSA can be as long as 4096 bits.
If we take each byte (8 bits) in the original buffer as one symbol, the formula can have 
512 symbols. 

\vspace*{6pt}
\textbf{Our Solution to Challenge b:}
We adopt multiple step Monte Carlo sampling to count the number of possible inputs
...
as $k_1, k_2, \ldots , k_n$. Suppose the address of the leakage site is $\addr{i}$,
...
\begin{enumerate}
      ...
      short constraints $F(c_{{addr}_i})$. The set containing the input variables of 
