...
experiments show that above approach suffers from unbearable costs, which
...
intermediate languages (IR). Because 
...
rules for every instruction. On the contrary,
IR typically has fewer instructions compared to
the original machine ISA\@. However, the IR layer designs, which
simplify the workload implementation those tools, 
also introduce significant overhead~\cite{217563}.
...
We argue that while it is a universal
