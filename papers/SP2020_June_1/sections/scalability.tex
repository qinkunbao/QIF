...
We use symbolic execution to get the function $f_i$ and model 
counting to get the number of items in $K^o$. However, both
symbolic execution and model counting are well-known for theirs
...
However, some pre-experiments show that above approach suffers from the unbearable cost, which impede its usage
...
\begin{itemize}
    ...
    \item Monte Carlo Sampling  (Challenge b)
\end{itemize}
