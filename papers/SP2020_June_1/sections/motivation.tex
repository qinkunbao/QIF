...
The leakage $L_{Pko}$ based on the oberservation is
    $$L_{Pko} = log_2{|K|} - log_2{|K^k|}$$
    where
    $$K^k = \{k^{'} | k^{'}{\in}K \ and \ P(k^{'}) = o \}$$
\end{mydef}
...
to achieve the attack. For example, the controlled-side channel attack~\cite{7163052, 191010}, the author 
...
Suppose one program has two side-channel vulnerabilities A and B, which leaks $L_A$ and $L_A$ bits
