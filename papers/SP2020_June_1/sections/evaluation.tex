...
Bit-slicing is a software technology which is capable to implement algorithms with high efficiency and constant running time. 
% Citation Needed
This apprach can immune cryptographic algorithms to cache and timing-related side channel attacks. The basic concept is to express a function in terms of single-bit logical operations – AND, XOR, OR, NOT, etc. – as if implementing a logic circuit in hardware. These operations are then carried out for multiple instances of the function in parallel, using bitwise operations on a CPU. 
...
Matthew Kwan and Eli Biham first present DES implements based on bit-slicing. 
