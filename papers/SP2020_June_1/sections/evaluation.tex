...
and 4, respectively. At line 6, according to Definition~\ref{def}, the input set
$K$ is $[0,2^8-1]$, the observed input set $K^o$ is $[0,2^3-1]$. Thus, the
leakage $L_{\beta(k)\rightarrow o}$ based on the observation ($o$) is
...
Considering Cache-Collision Timing Attack described in
\cite{Bonneau11894063_16}, the probability of the leakage being observed is
decreased by fewer byte-wide for the lookup table. Thus theoretically, a one
byte-wide lookup table (See Figure~\ref{fig:one_byte_table}) will leaks less
information than a four byte-wide one (See Figure~\ref{fig:four_byte_table}).
\tool{} reports one leakage site for each lookup table. For the four byte-wide
one, the number of leaked bits is 24.2 and the one byte-wide leaks 12.0 bits.