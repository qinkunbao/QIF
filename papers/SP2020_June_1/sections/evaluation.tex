...
probability of leakage decreases when the lookup table entry or element size is
smaller. We take AES an example, for the last round of encryption, the algorithm
uses a specialized 4 byte-wide table for the transformation, of which only one
byte actually contributes to the result. Hence, a significant method to reduce
...
\tool{} reports one leakage site for each lookup table, with 12.0 bits for the
table with smaller entries and 24.2 bits for the table with bigger entries,
