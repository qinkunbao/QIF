...
with $k_1, k_2, ... , k_n$. Suppose the address of the leakage site is $\mathit{addr}_i$,
we use $C_{\mathit{addr}_i}$ to denote the constraint. For multiple leakage sites, 
...
the following constraints: $C_{\mathit{addr}_1}, C_{\mathit{addr}_2}, ..., C_{\mathit{addr}_n}$ respectively. 
The total leakage has the constraint 
$F_{c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n}} = C_{\mathit{addr}_1} \land C_{\mathit{addr}_2} \land C_{\mathit{addr}_3}
\land ... \land C_{\mathit{addr}_n}$. The problem of estimating the total leaked information 
can be reduced to the problem of counting the number of different solutions 
that satisfies the constraint $F_{c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n}}$. 
...
However, despite the two problems. We also observe the constrain $F_(c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n})$
has the following characteristics:
\begin{enumerate}
      \item $F_(c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n})$ is the conjunction of several
      short constraints $F_{c_{{addr}_i}}$. The set contaning the input variables of 
      $F_{c_{{addr}_i}}$ is the subset of the input variables of $F_(c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n})$.
...
$F_(c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n})$ into several independent constraint groups. After
...
the constraint $F_{c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n}}$ into several 
independent constraints. There are many partitions. For our project, 
we are interested in the following one.

\begin{definition}
      \label{Goodpartition}
      For the constraint $F_{c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n}}$ and 
      the constraint group
      $C_{g1}, C_{g2}, ..., C_{gm}$, we call  $C_{g1}, C_{g2}, ..., C_{gm}$
      is the maximum independent partition of $F_{c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n}}$ iff
      \begin{enumerate}
            \item $C_{g1} \land C_{g2} \land ... \land C_{gm} = F_{c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n}}$
...
$$|K_{F_{c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n}}}| = |K_{C_{g1}}|*|K_{C_{g2}}|*...*|K_{C_{gn}}|$$
\end{theorem}
...
of the $F_{c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n}}$.

\IncMargin{1em}
\begin{algorithm}[h]
\DontPrintSemicolon
\SetKwInOut{Input}{input}\SetKwInOut{Output}{output}
\Input{$F_{c_{\mathit{addr}_1},c_{\mathit{addr}_2},...,c_{\mathit{addr}_n}} = C_{{addr}_1} \land C_{{addr}_2} \land ... \land C_{{addr}_m}$}
\Output{The Maximum Independent Partition of $G = \{C_{g1}, C_{g2}  , ...,  C_{gm} \}$ }
\For{$i\leftarrow 1$ \KwTo $n$}
{
   $S_i$ $\leftarrow$ $S(C_{\mathit{addr}_i})$ \;
   \For{$C_{gi} \in G$} 
   {
   ...
   \If{$s \neq \emptyset$}
   {
      $C_{gi} = C_{gi} \land C_{\mathit{addr}_i}$ \;
      $break$ \;
   }
